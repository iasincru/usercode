...
class Plotter {
 ...
 public:
  ...
  void MakeTable();
  void Table(TH1* , TH1* , TString, TString);
  void PlotXSec();
...
//         if (intbinsX[i] != intbinsY[i]) std::cout << "ARGH!!!!!\n";
    ...
    Table(h_DiffXSec, h_GenDiffXSec, name, channel);
...
void Plotter::Table(TH1 *h1, TH1 *h2, TString str, TString chan){
    
    int nbins1 = h1->GetNbinsX();
    int nbins2 = h2->GetNbinsX();

    if (nbins1 != nbins2){
    cout<<"WARNING: You are trying to print out the histogram content for different binned histograms"<<endl;
    return;
    }

    str.Prepend("Table__"+chan+"_");
    str.Append(".txt");
    ofstream myfile;
    myfile.open (str);
    myfile<<setprecision(12);
    myfile << "Histogram Data: "<<name<< h1->GetName()<<endl;
    myfile << "-------------------------------------------"<<endl;
    myfile <<"Bin"<<setw(12)<<" BinCenter"<<setw(14)<<" dSigma/dX"<<endl;
    for (int i=-1; i<nbins1+1; ++i){
        myfile<<i<<" "<<setw(12)<<h1->GetBinCenter(i+1)<<" "<<setw(12)<<h1->GetBinContent(i+1)<<endl;
    }
    myfile << "Histogram Madgraph: "<<name<< h2->GetName()<<endl;
    myfile << "-------------------------------------------"<<endl;
    myfile <<"Bin"<<setw(12)<<" BinCenter"<<setw(14)<<" dSigma/dX"<<endl;
    for (int i=-1; i<nbins1+1; ++i){
        myfile<<i<<" "<<setw(12)<<h2->GetBinCenter(i+1)<<" "<<setw(12)<<h2->GetBinContent(i+1)<<endl;
    }
    myfile.close();
