...
{   
    ...
    while(!infile.eof()){
        ...
        for(const auto& selectedChannel : channels){
            ...
            if(dy){
                ztop::RecoilCorrector *recoilCorrector = new ztop::RecoilCorrector(common::CMSSW_BASE()+"/src/TopAnalysis/ZTopUtils/data/");
                recoilCorrector->setFiles("METrecoil_Fits_DataSummer2013.root", "METrecoil_Fits_MCSummer2013.root");
                selector->SetMetRecoilCorrector(recoilCorrector);
