...
//     RecoPlot =  fileReader->GetClone<TH1D>(ftemp, "aRecHist");
    ...
    SmoothSyst *smoothSyst = new SmoothSyst("UnfoldingResults", "UnfoldingResults",7);
    for (size_t it=0; it<vec_systematic.size(); it++ )
    {
        smoothSyst->SetChannel(Channel);
        smoothSyst->SetVariable(name);
        bool validSyst = smoothSyst->SetSystematicAndCreateSingleHistograms(vec_systematic.at(it));
        if (!validSyst) continue;
        smoothSyst->FillSingleHistoFitAndText("SingleHistoAndAllFits");
    }
    delete smoothSyst;
