...
const std::vector<const char*> VectorOfValidSystematics 
    {"Nominal",
    "JER_UP_P1SIGMA", "JER_UP_P15SIGMA", "JER_UP_P2SIGMA", "JER_DOWN_M1SIGMA", "JER_DOWN_M15SIGMA", "JER_DOWN_M2SIGMA", 
    "PU_UP", "PU_DOWN",
        ...
        if (doDiffXSPlotOnly) {
            for (auto channel:channels){
                ...
                for (size_t sys=0; sys<syst.size(); ++sys){
                    ...
                    if(syst.at(sys) == TString("Nominal") || syst.at(sys).Contains("DOWN") || 
                        ( valid_sys.size() && syst.at(sys).Contains(valid_sys.at(valid_sys.size()-1))) )
                    {
                        continue;
                    }
                    if (syst.at(sys).Contains("_P2") || syst.at(sys).Contains("_P1") ||
                        syst.at(sys).Contains("_M2") || syst.at(sys).Contains("_M1") ||
                        syst.at(sys).Contains("UP"))
                        {
                            //syst.at(sys).Resize(syst.at(sys).First("UP"));
                            ///very ugly method to remove the type of variation: UP or DOWN
                            while (syst.at(sys).Contains("UP")) { syst.at(sys).Resize(syst.at(sys).Length()-1); }
                            if (syst.at(sys).Contains("_U")) {syst.at(sys).Resize(syst.at(sys).Length()-1);}
                        }
                    valid_sys.push_back(syst.at(sys));
                };
